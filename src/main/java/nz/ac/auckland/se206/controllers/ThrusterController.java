...
public class ThrusterController {
   ...
   * the button's cycle number is not equal to the random color number generated by the game state. 
   * If the button's cycle number is equal to the random color number, the button's timeline is paused 
   * and the button activation counter is incremented. Finally, the completion of the game is checked.
   */
  ...
  /**
   * Begins the thruster mini-game if the player has collected the blueprint and solved the color puzzle.
   * If the player is missing either the blueprint or color key, an error message is displayed.
   * Once the mini-game is activated, the repair button is hidden.
   */
   ...
   * updates the progress bar, sets the win game collision box to visible, starts the win game flash, 
   * sets the thrusters to invisible and disabled, sets the outside image to visible, 
   * sets the outside broken image to invisible, and sets the complete image to visible.
   */
